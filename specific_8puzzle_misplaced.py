# A* algorithm solution for a specific 8 puzzle problem using misplaced tiles
# / hamming distance as heuristic function
...
# Calulcate the misplaced tiles distance
def misplaced_tiles(s, g):
    return np.sum(s != g) - 1
...
def misplacedSolve(board, goal):
    ...
    # Calculate heuristic of n using misplaced tiles function
    heuristicn = misplaced_tiles(coordinate(board), goal_coordinate)
    ...
    while True:
        ...
        for m in moves:
            if loc not in m['pos']:
                ...
                hn = misplaced_tiles(coordinate(success_t), goal_coordinate)
                state = np.append(
                    state,
...
def main():
    ...
    state, explored = misplacedSolve(board, goal)
